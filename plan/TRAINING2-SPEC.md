...
6. **Open questions for Duke:** (i) A vs B preference if both pass gates (A is simpler; B is theoretically right about the beta); (ii) is `MISS_WEIGHT > 0` (penalizing missed winners — approached-but-passed buys **and** avoided run-ups) acceptable as a behavior prior, or should misses be unsupervised? (`AVOID_CORRECT_WEIGHT = 0.5` for correct avoids is pinned-by-spec and swept, not a Duke decision); (iii) confirm the never-seen 20-name basket before Phase A starts (it must stay unseen by *all* training runs, including sweeps); (iv) appetite for whole-fly eval wall-clock (~2 overnight runs per eval round).
