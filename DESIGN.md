...
- **v0.7 (2026-09-16):** Ratified by Duke: plan/TRAINING2-SPEC.md amendments A1–A9. Objective restated: NOT alpha-maximization — an active trader that chooses picks based on information, not randomly; A (supervised entry-forecast reward) vs B (advantage gating) decided empirically in Phase 0 on the stripped lane, G-A gates as referee. Package: declared basket files (40–60 train + held-out eval baskets, universe stays plume-filtered, D3); identity attenuation knob `id_scale ∈ [0,1]` (D5; anchor sniff scales too, silent at 0); per-encounter dopamine generalized — supervised entry-forecast credit (forward return after the decision bar, credited at close-fill/settle only) + avoid-side credit (correct avoids rewarded at 0.5, missed run-ups punished at 0.25), single-credit last-credit-wins, optionally an advantage baseline (state-conditional EMA persisted in the artifact); daily pooled ritual ablatable (`daily_observe`); calibrated asymmetric gate gains (1.5/1.0 Phase-A start, D6); mechanical exits extended (trailing/ATR stops, T-1 exit-forecast path; taste PAM/PPL1 documented readout-inert pending D15); hard anti-lookahead rule (credit only from bars strictly after the decision bar, audited by test); eval protocol ≥20 days/period × ≥2 periods × ≥2 baskets (one never-seen), paired-t primary; artifact meta contract + eval-side assertion; turnover guard [0.5×, 2.0×] of reference. `MISS_WEIGHT=0.25` ratified; never-seen 20-name eval basket frozen in `baskets/eval20-neverseen.txt` (all listed pre-2024, disjoint from every trained basket) at Phase-0 close. v0.6 stays source of truth for anything not amended here.
